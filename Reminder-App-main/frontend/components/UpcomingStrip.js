...
export default function UpcomingStrip({ tasks, colors }) {
  ...
  const upcoming = tasks
    ...
    .map(t => ({ ...t, due: new Date(`${t.date}T${t.time}:00`) }))
    .filter(t => !isNaN(t.due) && t.due >= now)  // guard invalid dates
    ...
    <View style={[styles.container, { backgroundColor: colors.accentLight, borderColor: colors.border }]}>
      ...
      {upcoming.map(task => (
        <View key={task._id} style={styles.taskRow}>
          ...
          <Text style={[styles.timeLabel, { color: colors.accent }]}>
            {getTimeLabel(task.due)}
          </Text>
        </View>
      ))}
    </View>
